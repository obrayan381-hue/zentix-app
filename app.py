...
def forzar_sidebar_abierto():
    ...
    (function () {
        ...
        function applyDesktopSidebarStyles(sidebar) {
            if (!sidebar) return;
        ...
        function forceSidebarOpen() {
            ...
            if (window.innerWidth >= 769 && (expanded === "false" || tooSmall)) {
                if (collapsedControl) {
            ...
            applyDesktopSidebarStyles(sidebar);
        }

        forceSidebarOpen();
        setTimeout(forceSidebarOpen, 150);
        ...
        window.addEventListener("resize", forceSidebarOpen);
    })();
    </script>
    """, height=0)
...
with st.sidebar:
    ...
    st.markdown("<div class='sidebar-nav-note'>Toda la navegación quedó fija en este panel lateral para escritorio y móvil.</div>", unsafe_allow_html=True)
    pagina_sidebar = st.radio(
        "Navegación principal",
        paginas_disponibles,
        index=paginas_disponibles.index(st.session_state.pagina),
        key="zentix_sidebar_nav",
        label_visibility="collapsed"
    )
